...
async function chargerTousMembres() {
  ...
  tbody.innerHTML = data.length
    ? ''
    : '<tr><td colspan="5">Aucun membre enregistré.</td></tr>';
  ...
  data.forEach((profil) => {
    ...
    const action =
      profil.role === 'admin'
        ? '<span class="help-text">Compte admin</span>'
        : `
          <button
            class="btn-delete"
            data-action="supprimer-membre"
            data-id="${profil.id}"
          >
            Supprimer
          </button>
...
async function actionMembre(event) {
  ...
  if (bouton.dataset.action !== 'supprimer-membre') {
    return;
  }
